...
<section class="content">
    ...
    <!-- /.row (main row) -->
      ...
      case 'add':
      if (isset($_POST['add'])) {
	    if  ($_POST['saldo_akhir']>='0'){	
	   			
              $query .= mysql_query("INSERT INTO petty_cash VALUES ('','$_POST[kd_byr]','$_POST[tgl]','$_POST[nama_akun]','$_POST[nominal]')");
		       echo "<script> alert('data berhasil disimpan');window.location='home.php?pg=pettycash&act=view'</script>";
              }else {
			   echo "<script type='text/javascript'>  alert('Gagal, Karna saldo bank tidak boleh minus !');</script>";
			 }
                  ...
                  <div class="box-body">
                   ...
                   <label for="exampleInputEmail1">Kode Transaksi</label>
					 ...
					 <div class="form-group">
                      ...
                      <input type="date" class="form-control" required data-fv-notempty-message="Tidak boleh kosong" id="tgl" name="tgl" placeholder="MM/DD/YYY" >
                    </div>
					  ...
					  <div class="form-group">
					   ...
					   $d = mysql_fetch_array(mysql_query("SELECT saldo FROM bank WHERE id='2'"));
					   ?>
                      <input type="text" class="form-control" id="saldo" name="saldo" onKeyUp="sum();" placeholder="input nominal"value="<?php echo $d['saldo'];?>" >
					  <input readonly class="form-control" id="saldo_akhir" name="saldo_akhir" onKeyUp="sum();" >
                    </div>
					...
					<script>
						  function sum() {
							...
							var txt1 = document.getElementById('saldo').value;
							var txt2 = document.getElementById('nominal').value;
							var txt3 = document.getElementById('saldo_akhir').value;
							
							
												
							var result1 = (txt1)-(txt2);
      						if (!isNaN(result1)) {
         					document.getElementById('saldo_akhir').value = result1;
							}
