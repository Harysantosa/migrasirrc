...
function kd_lp_auto()
	{
		...
		if($m['notrans'] <> NULL)
		{
			...
			if(strlen($kd) == 1)
			{
				$no = "LAPPROD".date('dmy')."0000".$kd;
			}
			elseif (strlen($kd) == 2) {
				$no = "LAPPROD".date('dmy')."000".$kd;
			}
			elseif (strlen($kd) == 3) {
				$no = "LAPPROD".date('dmy')."00".$kd;
			}
			elseif (strlen($kd) == 4) {
				$no = "LAPPROD".date('dmy')."0".$kd;
			}
			else {
				$no = "LAPPROD".date('dmy').$kd;
			}
		}
		else
		{
			$no = "LAPPROD".date('dmy')."00001";
		}

		return $no;
	}

	function kd_dry_auto()
	{
		$sql = mysql_query("SELECT MAX(RIGHT(no_lap,5)) AS notrans
							FROM lapdry WHERE id");
		$m = mysql_fetch_assoc($sql);
		...
		if($m['notrans'] <> NULL)
		{
			...
			if(strlen($kd) == 1)
			{
				$no = "LAPDRY".date('dmy')."0000".$kd;
			}
			elseif (strlen($kd) == 2) {
				$no = "LAPDRY".date('dmy')."000".$kd;
			}
			elseif (strlen($kd) == 3) {
				$no = "LAPDRY".date('dmy')."00".$kd;
			}
			elseif (strlen($kd) == 4) {
				$no = "LAPDRY".date('dmy')."0".$kd;
			}
			else {
				$no = "LAPDRY".date('dmy').$kd;
			}
		}
		else
		{
			$no = "LAPDRY".date('dmy')."00001";
		}
...
function kd_ppic_auto()
	{
		$sql = mysql_query("SELECT MAX(RIGHT(ppic_id,8)) AS notrans
...
function kd_ppic2_auto()
	{
		...
		if($m['notrans'] <> NULL)
		{
			...
			if(strlen($kd) == 1)
			{
				$no = "PPICPLAN".date('dmy')."0000".$kd;
			}
			elseif (strlen($kd) == 2) {
				$no = "PPICPLAN".date('dmy')."000".$kd;
			}
			elseif (strlen($kd) == 3) {
				$no = "PPICPLAN".date('dmy')."00".$kd;
			}
			elseif (strlen($kd) == 4) {
				$no = "PPICPLAN".date('dmy')."0".$kd;
			}
			else {
				$no = "PPICPLAN".date('dmy').$kd;
			}
		}
		else
		{
			$no = "PPICPLAN".date('dmy')."00001";
		}
