...
function kd_qa_auto()
	{
		$sql = mysql_query("SELECT MAX(RIGHT(kode,5)) AS notrans
							FROM approval_rm WHERE tgl = '".date('Y-m-d')."'");
		$m = mysql_fetch_assoc($sql);
		...
		if($m['notrans'] <> NULL)
		{
			...
			if(strlen($kd) == 1)
			{
				$no = "QA".date('dmy')."0000".$kd;
			}
			elseif (strlen($kd) == 2) {
				$no = "QA".date('dmy')."000".$kd;
			}
			elseif (strlen($kd) == 3) {
				$no = "QA".date('dmy')."00".$kd;
			}
			elseif (strlen($kd) == 4) {
				$no = "QA".date('dmy')."0".$kd;
			}
