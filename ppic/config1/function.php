...
function kd_trans_auto()
	{
		$sql = mysql_query("SELECT MAX(RIGHT(id,5)) AS notrans
...
function kd_ppic_auto()
	{
		$sql = mysql_query("SELECT MAX(RIGHT(ppic_id)) AS no
							FROM ppic_plan order by id");
		$m = mysql_fetch_assoc($sql);

		$no = 0;
		if($m['no'] <> NULL)
		{
			$kd = number_format($m['no='],0) + 1;
			if(strlen($kd) == 1)
			{
...
function kd_ppic2_auto()
	{
		...
		if($m['notrans'] <> NULL)
		{
			...
			if(strlen($kd) == 1)
			{
				$no = "Pn".date('dmy')."0000".$kd;
			}
			elseif (strlen($kd) == 2) {
				$no = "Pn".date('dmy')."000".$kd;
			}
			elseif (strlen($kd) == 3) {
				$no = "Pn".date('dmy')."00".$kd;
			}
			elseif (strlen($kd) == 4) {
				$no = "Pn".date('dmy')."0".$kd;
			}
